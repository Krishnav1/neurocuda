"""
Demo B v3 — CartPole ANN→SNN Conversion with Weight Rescaling
===============================================================
Root cause (found by debug_cartpole_gap.py):
  LIF outputs are capped at threshold, but ReLU is unbounded.
  After weight transfer, LIF2 sees inputs ~6x smaller than ReLU2.
  Q-values collapse from 112 → 18, Q correlation only 0.24.

Fix:
  1. Transfer fc weights from ANN
  2. Measure LIF vs ReLU output scale per layer
  3. Rescale NEXT layer's weights to compensate
  4. BPTT fine-tune from this much better starting point

Usage: python examples/demo_b_conversion_v3.py
"""
...
def rescale_weights_for_lif(snn_model, ann_model, states, T=16):
    """Rescale fc weights to compensate for LIF output being smaller than ReLU.

    For each layer i:
      k_i = E[ReLU_i(output)] / max(E[LIF_i(output)], epsilon)
      fc_{i+1}.weight *= k_i
      fc_{i+1}.bias   *= k_i

    This ensures the next layer sees inputs of similar scale to what the
    ANN weights were trained for.
    """
    ann_model.eval()
    snn_model.eval()
    ...
    with torch.no_grad():
        ...
        # --- ANN activations ---
        ann_h1 = torch.relu(ann_model.fc1(states))   # (N, 128)
        ...
        for t in range(T):
            h1 = snn_model.lif1(snn_model.fc1(states))
            lif1_sum += h1
            h2 = snn_model.lif2(snn_model.fc2(h1))
            lif2_sum += h2
        lif1_avg = lif1_sum / T
        lif2_avg = lif2_sum / T
        ...
        eps = 1e-4
        k1 = ann_h1.mean() / max(lif1_avg.mean(), eps)
        k2 = ann_h2.mean() / max(lif2_avg.mean(), eps)

        # Clamp to reasonable range
        k1 = max(0.5, min(k1, 20.0))
        k2 = max(0.5, min(k2, 20.0))

        print(f"  Rescaling factors: k1={k1:.3f} (ReLU1={ann_h1.mean():.3f} / "
              f"LIF1={lif1_avg.mean():.3f})")
        print(f"                      k2={k2:.3f} (ReLU2={ann_h2.mean():.3f} / "
              f"LIF2={lif2_avg.mean():.3f})")

        # --- Apply rescaling ---
        snn_model.fc2.weight.data *= k1
        snn_model.fc2.bias.data *= k1
        snn_model.fc3.weight.data *= k2
        snn_model.fc3.bias.data *= k2

    return snn_model, {"k1": k1, "k2": k2}


# ===========================================================================
# Verify rescaling improved Q correlation
# ===========================================================================

def verify_q_correlation(ann_model, snn_model, states, label=""):
    """Measure Q-value correlation between ANN and SNN."""
    ann_model.eval()
    snn_model.eval()
    with torch.no_grad():
        ann_q = ann_model(states)
        reset_spiking(snn_model)
        snn_q = snn_model(states)
        corr = torch.corrcoef(torch.stack([ann_q.flatten(), snn_q.flatten()]))[0, 1].item()
        err = (ann_q - snn_q).abs().mean().item()
        if label:
            print(f"  {label}: Q corr={corr:.4f}, Q err={err:.2f}, "
                  f"ANN Q mean={ann_q.mean():.1f}, SNN Q mean={snn_q.mean():.1f}")
    return corr, err
...
if __name__ == "__main__":
    ...
    print("CartPole ANN→SNN Conversion v3 — Weight Rescaling + BPTT")
    print("=" * 60)
    t_start = time.time()
    ...
    torch.save(ann_model.state_dict(), "./checkpoints/demo_b_ann_dqn_v3.pt")

    # --- 2. Create Spiking DQN + Transfer + Rescale ---
    print("\nSTEP 2: Weight Transfer + LIF Scale Calibration")
    print("-" * 40)
    ...
    # Collect calibration states
    env_cal = gym.make("CartPole-v1")
    cal_states = []
    state, _ = env_cal.reset()
    for _ in range(200):
        cal_states.append(state)
        with torch.no_grad():
            s = torch.tensor(state, dtype=torch.float32, device=device).unsqueeze(0)
            action = ann_model(s).argmax(1).item()
        state, _, terminated, truncated, _ = env_cal.step(action)
        if terminated or truncated:
            state, _ = env_cal.reset()
    env_cal.close()
    cal_tensor = torch.tensor(np.array(cal_states), dtype=torch.float32, device=device)

    # Verify before rescaling
    print("  Before rescaling:")
    corr_before, err_before = verify_q_correlation(
        ann_model, snn_model, cal_tensor, "  ")

    # Rescale fc weights
    snn_model, rescale_info = rescale_weights_for_lif(
        snn_model, ann_model, cal_tensor, T=16)

    # Verify after rescaling
    print("  After rescaling:")
    corr_after, err_after = verify_q_correlation(
        ann_model, snn_model, cal_tensor, "  ")

    improvement = err_before - err_after
    print(f"\n  Q error reduction: {err_before:.1f} → {err_after:.1f} "
          f"({improvement:.1f} absolute, {100*improvement/err_before:.0f}%)")

    # Evaluate BEFORE fine-tuning
    snn_mean_before, snn_solved_before = evaluate(snn_model, is_spiking=True)
    print(f"\n  Spiking DQN BEFORE fine-tune: Mean = {snn_mean_before:.1f}, "
          f"Solved = {snn_solved_before:.0f}%")

    # --- 3. Fine-tune Spiking DQN ---
    print("\nSTEP 3: Fine-Tune Spiking DQN (BPTT + Surrogate Gradient)")
    print("-" * 40)
    print("  Starting from rescaled weights — much closer to ANN policy")
    ...
    criterion = nn.MSELoss()
    replay = ReplayBuffer()
    epsilon = 0.5  # Lower — rescaled weights are already good
    steps_done = 0
    ft_rewards = []
    ...
    # --- Summary ---
    print(f"\n{'='*60}")
    print("CART-POLE CONVERSION v3 — SUMMARY")
    print("=" * 60)
    print(f"""
  Method:         Weight transfer + LIF scale calibration + BPTT FT
  ANN DQN:        Mean Reward = {ann_mean:.1f}, Solved = {ann_solved:.0f}%
  Spiking DQN:
    Before rescale: Q err = {err_before:.1f}
    After rescale:  Q err = {err_after:.1f} ({100*improvement/err_before:.0f}% reduction)
    Before FT:     Mean = {snn_mean_before:.1f}, Solved = {snn_solved_before:.0f}%
    After FT:      Mean = {snn_mean:.1f}, Solved = {snn_solved:.0f}%
  Sparsity:        {sparsity:.2f}%
  Rescale factors: k1={rescale_info['k1']:.3f}, k2={rescale_info['k2']:.3f}
