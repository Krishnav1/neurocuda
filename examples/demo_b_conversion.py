"""
Demo B — CartPole ANN→SNN Conversion via Weight Transfer + BPTT Fine-Tuning
=============================================================================
Pipeline:
  1. Train ANN DQN (ReLU) — standard PyTorch RL
  2. Create Spiking DQN (LIF), transfer fc weights from ANN
  3. Fine-tune Spiking DQN with surrogate gradient BPTT
  4. Measure: control performance, sparsity, efficiency

Unlike Demo A (where QCFS calibrates IF thresholds), CartPole has no BatchNorm
and no natural temporal dimension. The conversion uses weight transfer:
ANN fc weights → Spiking DQN fc weights, then BPTT adapts them to binary spikes.

Usage: python examples/demo_b_conversion.py
"""
...
if __name__ == "__main__":
    ...
    print("CartPole ANN→SNN Conversion via Weight Transfer + BPTT")
    print("=" * 60)
    t_start = time.time()
    ...
    torch.save(ann_model.state_dict(), "./checkpoints/demo_b_ann_dqn.pt")

    # --- 2. Create Spiking DQN with weight transfer ---
    ...
    print("  LIF neurons: fresh (thresh=1.0, beta=0.5)")

    # Evaluate BEFORE fine-tuning
    snn_mean_before, snn_solved_before = evaluate(snn_model, is_spiking=True)
    print(f"  Spiking DQN BEFORE fine-tune: Mean = {snn_mean_before:.1f}, Solved = {snn_solved_before:.0f}%")

    # --- 3. Fine-tune Spiking DQN ---
    print("\nSTEP 3: Fine-Tune Spiking DQN (BPTT + Surrogate Gradient)")
    print("-" * 40)
    print("  (Using ANN weights as initialization — much faster than random init)")
    ...
    criterion = nn.MSELoss()
    replay = ReplayBuffer()
    epsilon = 0.3  # Start lower — ANN weights are already good
    steps_done = 0
    ft_rewards = []

    for episode in range(300):  # Much fewer than 600 — weights are pre-trained
        ...
        if (episode + 1) % 50 == 0:
            print(f"  FT Ep {episode+1}: Avg100 = {avg_100:.1f}")
    ...
    # --- Summary ---
    print(f"\n{'='*60}")
    print("CART-POLE CONVERSION SUMMARY")
    print("=" * 60)
    print(f"""
  Method:         ANN DQN weights → Spiking DQN → BPTT fine-tune
